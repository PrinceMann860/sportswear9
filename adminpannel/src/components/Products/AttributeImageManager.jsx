...
import { Upload, Trash2, X, Image as ImageIcon, CreditCard as Edit, Save, Star, StarOff } from 'lucide-react';
...
const AttributeImageManager = ({ attributeValue, onClose, onImagesUpdated }) => {
  ...
  useEffect(() => {
    // Initialize with existing images from the attribute
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // Upload images for this specific attribute value
      ...
      onImagesUpdated();
      onClose(); // Close modal to refresh parent component
    } catch (error) {
  ...
  const handleSetMainImage = async (image) => {
    try {
      // First, unset all other images as main
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-8">
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-400 transition-colors bg-gray-50 hover:bg-blue-50">
            <input
              type="file"
              multiple
              accept="image/*"
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            {images.length > 0 && (
              <span className="text-sm text-gray-500">
                Click on an image to edit its details
              </span>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {images.map((image, index) => (
                <div key={image.id || index} className="relative group bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  <div className="relative">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-2">
                      <button
                        onClick={() => handleEditImage(image)}
                        className="text-white hover:text-blue-300 transition-colors bg-blue-600 p-2 rounded-full"
                      ...
                      <button
                        ...
                        className="text-white hover:text-yellow-300 transition-colors bg-yellow-600 p-2 rounded-full"
                        title={image.is_main ? "Remove as main image" : "Set as main image"}
                      >
                        {image.is_main ? <StarOff className="h-4 w-4" /> : <Star className="h-4 w-4" />}
                      </button>
                      <button
                        onClick={() => handleDeleteImage(image)}
                        className="text-white hover:text-red-300 transition-colors bg-red-600 p-2 rounded-full"
                        title="Delete image"
                      >
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  />
                  <div 
                    className="w-full h-32 bg-gray-200 rounded border border-gray-200 mb-4 flex items-center justify-center text-gray-400"
                    style={{ display: 'none' }}
                  >
                ...
                <div>
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-700">Set as main image for this attribute</span>
                  </label>
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  onClick={handleUpdateImageMetadata}
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-200">
          <div className="text-sm text-gray-500">
            {images.length > 0 && (
              <span>
                {images.filter(img => img.is_main).length > 0 ? '1 main image, ' : 'No main image set, '}
